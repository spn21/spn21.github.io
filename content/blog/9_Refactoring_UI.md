...
颜色：在实践中，只需要关注不同颜色给人的感觉就好（如蓝色——安全又熟悉，黄色——昂贵、精致）

边界半径：较小的圆角半径比较中性，较大的边框半径会让人感觉更加活泼，无边框半径则更显得严肃正式

**保持一致性更为重要，多种混合不如坚持其中一个**

语言文字也是界面设计的一部分，专业的语气和友善的语气具有一定的差别
...
阴影也需要一套体系，设计最大最小边界，然后填充颜色，中间部分阴影大小呈随线性变化
...
照片有时会比较动态（色彩明暗显著，不好在上面配文字）可以降低图像动态范围，使文字与背景的对比度更加一致

添加半透明效果，叠加到背景图像上：黑色叠加层降低亮部亮度，突出浅色文字；白色叠加层提亮暗部区域，突出深色文字

降低图像对比度，对整幅图像提亮或提暗，但也注意调整亮度进行补偿

还有一种方法，用单一颜色对图像着色，可以分三步效果：

1. 降低图像对比度，使画面更加平静
2. 对图像进行饱和处理，去除所有原有颜色
3. 使用混合模式，添加纯色填充

其中的纯色可以选择自己品牌的颜色

如果想保留背景图像中更多的动态效果，文字阴影可以很好地增强所需区域的对比度

想让它看起来更像是柔和的光晕，而不是真正的阴影，可以选择较大的模糊半径，而不添加任何类型的偏移

### Everything has an intended size

不要随便放大图标，可能会更加模糊，可以选择矢量图（SVG）调整尺寸，但是放大倍数增加也会导致其图像质量下降

如果只有小图标，可以尝试把他们包裹在另一个形状内，添加背景色，也是填充空间的一种可能

屏幕截图尽量不要缩小，会显得拥挤，可以在较小的屏幕下进行截图保存（如电脑截图换手机截图）

如果需要在小空间里面截取整个截图，可以尝试绘制一个简化版的用户界面，移除小字，采用简单线条

也不要随便缩小原有图标，会显示不清，最好是重新绘制一个极其简化的版本

### Beware user-uploaded content

用户上传图片时，对上传的各种类型图片，如果不采取处理会受制于用户

将图像放入固定的“容器”中，防止不同长宽比的图片显得凌乱，“裁剪”掉多余的部分

CSS 实现，只需要将图像设置一个背景图片，将 background-size 设置为 cover

用户上传的照片有可能与背景色接近，可以加边框/内框的阴影来解决这个问题，显得“内嵌”；也可以选择半透明边框，效果可能更好

## 0x08 Finishing Touches

### Supercharge the defaults

不一定需要添加各种新元素，强化已有的元素也是一定的作用（图标替换符号列表），√、🔒也是很好的通用选择，改变引号尺寸、颜色也是可以改变的一个点

链接，更改颜色和字体粗细，更高级的如粗体彩色自定义下划线，添加部分阴影覆盖文本

制作表单使用自定义复选框和单选按钮，添加不同色彩（以自己品牌颜色为主）

### Add color with accent borders

在卡片顶部添加一个小的边框就可以极大改善原来的单调性
...
更改背景颜色：突出某个面板/区分各部分页面

采用轻微的渐变效果显得更具活力，为了获得最佳效果，色调相差不超过30'

重复图案 参考这种设计模式[Hero Patterns](https://heropatterns.com/)

尽量降低背景和图案之间的对比度，确保可读性

添加简单的图形描述更适合用户理解阅读，而不是堆砌文字

更复杂一点的简化背景：简化的世界地图

就像使用全背景图案一样，最好保持较低的对比度，不做到干扰内容

### Don't overlook empty states

优先考虑空状态（如联系列表中没有联系人时）可以尝试添加图片或者插图吸引用户，善意引导下一步行动

如果是一个辅助用户界面（选项卡）这种项目，或者筛选器，可以考虑隐藏这些内容，突出下一步操作，引导成功后再展现

空状态是用户第一次与新产品或者功能交互的状态，第一印象很重要！

### Use fewer borders

用边框来分割两个元素不是唯一的方法，在一个界面有很多元素时会显得杂乱

可以采用阴影框勾画元素轮廓，同时保证阴影颜色与背景颜色不同

给相邻元素赋予两种不同颜色，可以考虑取消掉边框，将不同元素之间距离拉大也是区分彼此的好方法

### Think outside the box

大多数人对某些事物有了先入为主的观念，尝试一些新意

如下拉菜单，不一定只是白色，带有阴影效果的方框，里面夹杂一些链接
可以是分成几个部分的内容，使用多列布局，添加辅助文字、彩色插图、图标

表格也不一定只是一列列、一行行的具体数据
可以采用不同层级，突出重点，边缘繁琐，加入颜色丰富现有数据
...
```
The absolute best way to notice the little details that make a design look really polished is to recreate that design from scratch, without peeking at the
developer tools.
